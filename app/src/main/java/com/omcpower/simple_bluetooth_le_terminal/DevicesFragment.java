...
public class DevicesFragment extends ListFragment {
    ...
    private void connectToDevice(int position,int type) {
        stopScan();
        BluetoothUtil.Device device = listItems.get(position-1);
        Bundle args = new Bundle();
        BluetoothDevice bDevice = device.getDevice();
        String address = bDevice.getAddress();
        String name = device.getName();
        args.putString("device", address);
        args.putInt("userType", type);
        ...
        editor.putString(getString(R.string.deviceName), name);
        editor.putString(getString(R.string.deviceAddress), address);
        editor.putInt(getString(R.string.userType), type);
        //Log.d("dname",name);
        //Log.d("dname",name);
        editor.apply();
        Fragment fragment = new TerminalFragment();
    ...
    private void getSuperUsers(int position) {
        String macAddress = getMacAddress();
        //Toast.makeText(getActivity(), macAddress, Toast.LENGTH_LONG).show();
        Api api = RetrofitClient.getClient().create(Api.class);
        Results results = new Results(false,macAddress,"",0);
        //Call<Results> call = api.getValidUsers(results);
        Log.d("Device","servinput " +results.getDevice_id());

        Call<Results> call = api.getLoginUsers(results);
        call.enqueue(new Callback<Results>() {
            @Override
            public void onResponse(Call<Results> call, Response<Results> response) {
                Results myUsersList = response.body();
                String msg = response.message();
                Log.d("Device","servresponse " +response);
                if(response.isSuccessful()) {
                    Toast.makeText(getActivity(), "connected successfully"+myUsersList.isValid(), Toast.LENGTH_LONG).show();
                    if(myUsersList.isValid()) {
                        connectToDevice(position, myUsersList.getType());
    ...
    private void showPopup(String androidId) {
        SpannableStringBuilder builder = new SpannableStringBuilder();

        String red = "Please ask admin to allow Id :";
        SpannableString redSpannable= new SpannableString(red);
        redSpannable.setSpan(new ForegroundColorSpan(Color.RED), 0, red.length(), 0);
        builder.append(redSpannable);

        String white = androidId;
        SpannableString whiteSpannable= new SpannableString(white);
        whiteSpannable.setSpan(new ForegroundColorSpan(Color.GREEN), 0, white.length(), 0);
        builder.append(whiteSpannable);

        String blue = " to use the app";
